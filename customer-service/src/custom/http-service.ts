...
type IRA = IRequestAttributes | IRequestAttributes[]

interface IHttpService {
  get<T extends IRA>(optionAttributes: T): Promise<ReturnPromise<T>>
}

class HttpService implements IHttpService {
  async get<T extends IRA>(optionAttributes: T): Promise<ReturnPromise<T>> {
    const requestAttributes: (IRequestAttributes & { method: HTTP_METHOD })[] = []
    const requests: ApiResponsePromises = []
    if (Array.isArray(optionAttributes)) {
      optionAttributes.forEach((attr) => {
        requestAttributes.push({ ...attr, method: 'GET' })
      })
    } else {
    ...
    function processOptionAttr(attr: IRequestAttributes & { method: HTTP_METHOD }): void {
      const { method, url, query, body, params, headers, auth, timeout } = attr
          ...
          .catch((error) => {
            ...
            if (error instanceof AxiosError) {
              if (error.code === 'ECONNABORTED') {
                ...
                ['ENOTFOUND', 'ECONNRESET', 'ECONNREFUSED', 'EAI_AGAIN', 'ENETUNREACH'].includes(error.code || '')
              ) {
                result_code = 'ret=1'
...
async function requestHttp<T extends RA>(optionAttributes: T): Promise<ReturnPromise<T>> {
  const requests: ApiResponsePromises = []
...
export { IRequestAttributes, HttpService, IHttpService }
